...
class VoiceAnalyzer:
    ...
    def __init__(self, use_parallel=True, max_workers=None, output_dir=None, log_dir=None):
        ...
        self.visualizer = VoiceVisualizer(output_dir=output_dir)
    ...
    def analyze_audio(self, audio_file, generate_report=True, progress_callback=None):
        """
        分析单个音频文件
        ...
        Args:
            ...
            progress_callback: 进度回调函数，接收字典参数 {'stage': str, 'progress': int, 'message': str, 'current': int, 'total': int}
        
        Returns:
            dict: 分析结果
        """
        def emit_progress(stage, progress, message='', current=0, total=0):
        ...
        try:
            ...
            emit_progress('分割完成', 25, f'共分割为 {len(segments)} 个音频段')
            
            self.logger.info("[步骤3] 提取声学特征")
            print("\n[步骤3] 提取声学特征...")
            features_start = time.time()
            
            emit_progress('特征提取', 30, f'开始提取 {len(segments)} 个音频段的特征...')
            ...
            emit_progress('特征提取完成', 60, f'特征提取完成，耗时 {features_time:.2f}秒')
            print(f"  特征提取耗时: {features_time:.2f}秒")
            log_performance(self.logger, "特征提取", features_time)
            
            emit_progress('情绪分析', 70, '正在分析情绪...')
            self.logger.info("[步骤4] 分析情绪")
            print("\n[步骤4] 分析情绪...")
            emotion_result = self.emotion_analyzer.analyze_emotion(features_timeline[0])
            print(f"  主导情绪: {emotion_result['dominant_emotion']}")
            print(f"  置信度: {emotion_result['confidence']:.2%}")
            self.logger.info(f"  主导情绪: {emotion_result['dominant_emotion']}, 置信度: {emotion_result['confidence']:.2%}")
            
            emit_progress('情绪分析完成', 80, f"主导情绪: {emotion_result['dominant_emotion']}, 置信度: {emotion_result['confidence']:.2%}")
            
            self.logger.info("[步骤5] 分析情绪时间线")
            print("\n[步骤5] 分析情绪时间线...")
            timeline_result = self.emotion_analyzer.analyze_emotion_timeline(features_timeline)
            print(f"  整体主导情绪: {timeline_result['dominant_overall']}")
            print(f"  情绪变化次数: {timeline_result['num_changes']}")
            log_emotion_distribution(self.logger, timeline_result.get('distribution', {}))
            
            emit_progress('情绪时间线', 85, f"情绪变化 {timeline_result['num_changes']} 次")
            
            self.logger.info("[步骤6] 聚合特征")
            overall_features = self._aggregate_features(features_timeline)
            log_features(self.logger, overall_features)
            
            emit_progress('聚合特征', 90, '特征聚合完成')
            ...
            if generate_report:
                emit_progress('生成报告', 95, '正在生成分析报告...')
                self.logger.info("[步骤7] 生成分析报告")
                print("\n[步骤7] 生成分析报告...")
                report_paths = self._generate_reports(result, audio_file)
                result['report_paths'] = report_paths
                self.logger.info(f"  报告生成完成: {len(report_paths)}个文件")
            
            emit_progress('分析完成', 100, f'总耗时: {total_time:.2f}秒')
    ...
    def _generate_reports(self, result, audio_file):
        """
        生成分析报告
        ...
        acoustic_path = self.visualizer.plot_acoustic_features(
            result['overall_features'], audio_file
        )
        report_paths['acoustic_features'] = acoustic_path
        
        emotion_path = self.visualizer.plot_emotion_analysis(
            result['emotion_result'], audio_file
        )
        report_paths['emotion_analysis'] = emotion_path
        
        timeline_path = self.visualizer.plot_emotion_timeline(
            result['timeline_result'], audio_file
        )
        report_paths['emotion_timeline'] = timeline_path
        
        comprehensive_path = self.visualizer.generate_comprehensive_report(
            result['overall_features'],
            result['emotion_result'],
            result['timeline_result'],
            audio_file,
            {
                'serial_time': result.get('serial_time', 0),
                'parallel_time': result.get('features_extraction_time', 0),
                'speedup': result.get('speedup', 1)
            }
        )
        report_paths['comprehensive_report'] = comprehensive_path
