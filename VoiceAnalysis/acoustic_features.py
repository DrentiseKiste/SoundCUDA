...
class AcousticFeatureExtractor:
    ...
    def extract_f0_features(self, audio, sr):
        ...
        try:
            ...
            valid_f0 = f0[~np.isnan(f0) & ~np.isinf(f0)]
    ...
    def extract_formant_features(self, audio, sr):
        ...
        try:
            ...
            for i in range(1, len(spectrum) - 1):
                if spectrum[i] > spectrum[i-1] and spectrum[i] > spectrum[i+1]:
                    peaks.append((freq_bins[i], spectrum[i]))
            
            # 按幅度排序，取前几个峰值
            peaks.sort(key=lambda x: -x[1])
            peaks = [p[0] for p in peaks[:6] if 50 < p[0] < 4000]  # 限制合理的频率范围
            peaks.sort()
            
            # 设置共振峰值
            if len(peaks) >= 3:
                formant_features['f1'] = float(peaks[0])
                formant_features['f2'] = float(peaks[1])
                formant_features['f3'] = float(peaks[2])
                formant_features['f4'] = float(peaks[3]) if len(peaks) > 3 else 0
                ...
                formant_features['formant_dispersion'] = float(np.std(peaks[:3]))
                formant_features['formant_avg'] = float(np.mean(peaks[:3]))
                
                # 简单的带宽估计
                formant_features['b1'] = float(peaks[0] * 0.1)
                formant_features['b2'] = float(peaks[1] * 0.1)
                formant_features['b3'] = float(peaks[2] * 0.1)
            else:
                # 使用默认值
        ...
        # 最后检查，确保没有 NaN/Inf 值
        for key in formant_features:
            val = formant_features[key]
            if isinstance(val, float) and (np.isnan(val) or np.isinf(val)):
                formant_features[key] = 0
    ...
    def extract_energy_features(self, audio, sr):
        ...
        for key in energy_features:
            val = energy_features[key]
            if isinstance(val, float) and (np.isnan(val) or np.isinf(val)):
                energy_features[key] = 0
    ...
    def extract_quality_features(self, audio, sr):
        ...
        for key in quality_features:
            val = quality_features[key]
            if isinstance(val, float) and (np.isnan(val) or np.isinf(val)):
                quality_features[key] = 0
    ...
    def extract_temporal_features(self, audio, sr):
        ...
        for key in temporal_features:
            val = temporal_features[key]
            if isinstance(val, float) and (np.isnan(val) or np.isinf(val)):
                temporal_features[key] = 0
