...
class DenoiseTab(QWidget):
    ...
    def start_denoise(self):
        ...
        def denoise_task():
            audio, sr = librosa.load(self.parent.current_audio, sr=16000)
            self.sr = sr
            denoised = self.denoiser.denoise(audio, sr)
            self.denoised_audio = denoised
            return audio, denoised, sr
        
        self.worker = WorkerThread(denoise_task)
...
class FourierSeparationTab(QWidget):
    """傅里叶说话人分离标签页"""
    def __init__(self, parent):
        super().__init__(parent)
        self.parent = parent
        ...
        param_layout.addWidget(QLabel("分离方法:"), 0, 0)
        self.method_combo = QComboBox()
        self.method_combo.addItems(["聚类方法 (推荐)", "NMF (非负矩阵分解)", "ICA (独立成分分析)"])
        self.method_combo.setCurrentText("聚类方法 (推荐)")
        param_layout.addWidget(self.method_combo, 0, 1, 1, 2)
        
        param_layout.addWidget(QLabel("说话人数量:"), 1, 0)
        self.n_speakers_spin = QComboBox()
        self.n_speakers_spin.addItems(["自动估计 (推荐)", "2", "3", "4", "5", "6", "7", "8"])
        self.n_speakers_spin.setCurrentText("自动估计 (推荐)")
        param_layout.addWidget(self.n_speakers_spin, 1, 1, 1, 2)
        
        param_layout.addWidget(QLabel("FFT窗口大小:"), 2, 0)
        self.fft_size_combo = QComboBox()
        self.fft_size_combo.addItems(["1024 (短音频)", "2048 (中等)", "4096 (长音频)", "8192 (超长)"])
        self.fft_size_combo.setCurrentText("2048 (中等)")
        param_layout.addWidget(self.fft_size_combo, 2, 1, 1, 2)
        
        info_label = QLabel("💡 提示: 使用'聚类方法'和'自动估计'可获得最佳效果")
        info_label.setStyleSheet("color: #007bff; font-style: italic; padding: 5px;")
        param_layout.addWidget(info_label, 3, 0, 1, 3)
        ...
        self.save_button.setEnabled(False)
        button_layout.addWidget(self.separate_button)
        button_layout.addWidget(self.save_button)
        ...
        self.layout.addLayout(button_layout)
        self.layout.addWidget(self.progress_bar)
        self.layout.addWidget(QLabel("分离结果:"))
        self.layout.addWidget(self.result_text)
        self.layout.addWidget(QLabel("可视化分析:"))
        self.layout.addWidget(self.scroll_area)
    ...
    def start_separation(self):
        ...
        n_speakers = None
        n_speakers_text = self.n_speakers_spin.currentText()
        if not n_speakers_text.startswith("自动"):
            n_speakers = int(n_speakers_text)
        
        fft_size_text = self.fft_size_combo.currentText()
        fft_size = int(fft_size_text.split()[0])
        
        self.separate_button.setEnabled(False)
        self.progress_bar.setValue(0)
        self.result_text.clear()
        self.result_text.append("开始傅里叶说话人分离...")
        self.result_text.append(f"分离方法: {self.method_combo.currentText()}")
        self.result_text.append(f"说话人数量: {'自动估计' if n_speakers is None else n_speakers}")
        self.result_text.append(f"FFT窗口大小: {fft_size}")
        
        def separate_task():
            self.separator = FourierSpeakerSeparator(
                sample_rate=16000,
                n_fft=fft_size
            )
            result = self.separator.separate_speakers(
                self.parent.current_audio,
                method=method,
                n_speakers=n_speakers
            )
            return result
        
        self.worker = WorkerThread(separate_task)
        self.worker.finished.connect(self.separation_finished)
        self.worker.error.connect(self.separation_error)
        self.worker.start()
    
    def separation_finished(self, result):
        self.current_result = result
        self.result_text.append("\n" + "=" * 60)
        self.result_text.append("分离完成！")
        self.result_text.append("=" * 60)
        
        self.result_text.append(f"\n检测到 {result['n_speakers']} 个说话人")
        self.result_text.append(f"原始音频时长: {len(result['original_audio']) / result['sample_rate']:.2f} 秒")
        self.result_text.append(f"分离方法: {result['method'].upper()}")
        ...
        if result['method'] == 'clustering':
            self.result_text.append("\n📊 自动估计依据:")
            self.result_text.append("  - 使用轮廓系数、Davies-Bouldin指数和肘部法则综合判断")
            self.result_text.append("  - 聚类特征包括: MFCC、频谱质心、频谱对比度等")
        
        self.result_text.append("\n说话人特征:")
        for i, audio in enumerate(result['separated_audios']):
            features = self.separator.extract_speaker_features(audio)
            f0 = features.get('mean_f0', 0)
            
            gender_hint = ""
            if f0 > 200:
                gender_hint = " (可能是女声)"
            elif f0 > 0 and f0 < 150:
                gender_hint = " (可能是男声)"
            
            self.result_text.append(f"\n说话人 {i+1}{gender_hint}:")
            self.result_text.append(f"  基频: {f0:.2f} Hz")
            self.result_text.append(f"  能量: {features.get('mean_rms', 0):.4f}")
            self.result_text.append(f"  频谱质心: {features.get('mean_centroid', 0):.2f} Hz")
            self.result_text.append(f"  时长: {features.get('duration', 0):.2f} 秒")
        
        self.result_text.append(f"\n已分离出 {len(result['separated_audios'])} 个音频片段")
        ...
        self.progress_bar.setValue(100)
        self.parent.statusBar().showMessage(f"分离完成，检测到 {result['n_speakers']} 个说话人")
    ...
    def plot_results(self, result):
        ...
        self._add_chart_to_scroll(
            "原始音频",
            result['original_audio'][:len(result['original_audio'])],
            sr,
            result['magnitude'],
            sr,
            self.separator.hop_length,
            color='blue'
        )
        
        self._add_feature_timeseries_chart(result)
        
        for i in range(n_speakers):
            speaker_audio = result['separated_audios'][i]
            _, mag, _ = self.separator.compute_stft(speaker_audio)
    ...
    def _add_feature_timeseries_chart(self, result):
        """添加时间序列特征对比图"""
        group_box = QGroupBox("说话人特征时间序列对比")
        group_layout = QVBoxLayout()
        ...
        n_speakers = result['n_speakers']
        sr = result['sample_rate']
        colors = plt.cm.Set1(np.linspace(0, 1, max(n_speakers, 2)))
        
        hop_length = 512
        frame_duration = hop_length / sr
        
        for i, speaker_audio in enumerate(result['separated_audios']):
            audio_len = len(speaker_audio)
            n_frames = int(audio_len / hop_length)
            ...
            try:
                ...
                if len(f0) > n_frames:
                    ...
                    f0 = np.pad(f0, (0, n_frames - len(f0)), mode='constant')
                
                axes[0].plot(time_frames[:len(f0)], f0, color=colors[i], 
                           label=f'说话人 {i+1}', alpha=0.8, linewidth=1.2)
            except:
                pass
            
            rms = librosa.feature.rms(y=speaker_audio, hop_length=hop_length)[0]
            time_rms = np.arange(len(rms)) * frame_duration
            axes[1].plot(time_rms, rms, color=colors[i], 
                        label=f'说话人 {i+1}', alpha=0.8, linewidth=1.2)
            
            spec = np.abs(librosa.stft(speaker_audio, hop_length=hop_length))
            centroid = librosa.feature.spectral_centroid(S=spec, sr=sr)[0]
            time_centroid = np.arange(len(centroid)) * frame_duration
            axes[2].plot(time_centroid, centroid, color=colors[i], 
                        label=f'说话人 {i+1}', alpha=0.8, linewidth=1.2)
        
        axes[0].set_title('基频 (F0) 时间序列')
        axes[0].set_ylabel('频率 (Hz)')
        axes[0].legend(loc='upper right', fontsize=8)
        axes[0].grid(True, alpha=0.3)
        axes[0].set_xlim(left=0)
        
        axes[1].set_title('能量 (RMS) 时间序列')
        axes[1].set_ylabel('能量')
        axes[1].legend(loc='upper right', fontsize=8)
        axes[1].grid(True, alpha=0.3)
        axes[1].set_xlim(left=0)
        
        axes[2].set_title('频谱质心时间序列')
    ...
    def _add_chart_to_scroll(self, title, audio, sr, magnitude, mag_sr, hop_length, color='blue'):
        """添加单个图表到滚动区域"""
        group_box = QGroupBox(title)
        group_layout = QVBoxLayout()
        ...
        ax1.plot(time_audio, audio, color=color, linewidth=0.5)
    ...
    def save_results(self):
        if not self.current_result or not self.separator:
            self.parent.statusBar().showMessage("没有可保存的结果")
            return
        ...
        try:
            self.separator.save_separated_audios(self.current_result, output_dir)
            
            import json
            features_list = []
            for i, audio in enumerate(self.current_result['separated_audios']):
                features = self.separator.extract_speaker_features(audio)
                features['speaker_id'] = i + 1
                features_list.append(features)
            
            features_file = os.path.join(output_dir, 'speaker_features.json')
            with open(features_file, 'w', encoding='utf-8') as f:
                json.dump(features_list, f, indent=2, ensure_ascii=False)
            
            self.parent.statusBar().showMessage(f"结果已保存到: {output_dir}")
        except Exception as e:
            self.parent.statusBar().showMessage(f"保存失败: {str(e)}")
    ...
    def separation_error(self, error_msg):
        self.result_text.append(f"\n错误: {error_msg}")
        self.separate_button.setEnabled(True)
        self.parent.statusBar().showMessage(f"分离失败: {error_msg}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.fourier_separation_tab = FourierSeparationTab(self)
        self.tabs.addTab(self.fourier_separation_tab, "傅里叶分离")
